...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			if (songStarted && !endingSong)
			{
				...
				if (unspawnNotes.length == 0 && notes.length == 0 && FlxG.sound.music.time > (songLength - 100))
				{
		...
		var lerp = 1 - FlxMath.bound(elapsed * 30, 0, 1);

		iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.width, lerp)));
		iconP2.setGraphicSize(Std.int(FlxMath.lerp(150, iconP2.width, lerp)));
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daNote != null)
			...
			noteDiff = Conductor.safeZoneOffset / songMultiplier; // Assumed SHIT if no note was given
		var wife:Float = EtternaFunctions.wife3(-noteDiff, Conductor.timeScale / songMultiplier);
		...
		if (daRating != 'shit' || daRating != 'bad')
		{
			...
			comboSpr.graphic.destroyOnNoUse = false;
	...
	{
		super.stepHit();
		if (Math.abs(FlxG.sound.music.time - Conductor.rawPosition) > 20
			|| (SONG.needsVoices && Math.abs(vocals.time - Conductor.rawPosition) > 20))
		{
			resyncVocals();
		}
